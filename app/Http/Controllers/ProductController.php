...
class ProductController extends Controller
{
	...
	public function saveCart(Request $request)
	{
        ...
        if($product_detail->is_featured == 1 || $product_detail->id != 16){
            $price = $product_detail->quarter_special_price;
        }else{
	...
	{
        $get_product_id = DB::table('products')->where('slug', $slug)->first();
        
		$product = new Product;
		$product_detail = $product->where('id', $get_product_id->id)->first();
        $product_imagess = DB::table('product_imagess')->where('product_id', $id)->get();
		...
		return view('shop.detail', compact('product_detail', 'shops', 'att_id', 'att_model', 'product_imagess'));
	}
