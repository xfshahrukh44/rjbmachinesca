...
<style>
  ...
  .amount-wrapper h3, .amount-wrapper h2 {
      display: FLEX;
      justify-content: SPACE-BETWEEN;
      font-size: 22px;
      border-top: 2px solid white;
            ...
            <div class="col-md-5 col-lg-5 col-sm-5 col-xs-12">
                ...
                <div class="YouOrder">
                    ...
                    <div class="amount-wrapper">
                      <h2>Item Subtotal <span>${{ number_format($subtotal) }}</span></h2>
                      @php
                          $tax = App\Http\Traits\HelperTrait::returnFlag(1976);
                          $shipping_cost = App\Http\Traits\HelperTrait::returnFlag(1977);
                          $taxFinal = ($tax / 100) * $subtotal;
                          $subtotal += $taxFinal+$shipping_cost;
                      @endphp
                      <h2> Tax <span>({{ $tax }}%) ${{ number_format($taxFinal) }} </span></h2>
                      <h2> Shipping Cost <span>${{ number_format($shipping_cost) }} </span></h2>
                      <h3> Total Price <span>${{ number_format($subtotal +  $variation) }}</span></h3>
                    </div>
                </div>
                    ...
                    <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
                      <div class="card-body">
                        <div class="stripe-form-wrapper require-validation" data-stripe-publishable-key="{{ env('STRIPE_KEY') }}" data-cc-on-file="false">
                          ...
                          <div class="form-group">
                            <button class="btn btn-red btn-block" type="button" id="stripe-submit">Pay Now ${{ number_format($subtotal) }}</button>
                          </div>
                        </div>
                      </div>
                    </div>
